# -*- coding: utf-8 -*-
"""
Create BarZ Endpoint Reference Points Script

Creates RPs at the endpoints of BarZ instances only:
- Avoids duplicates using coordinate precision
- Creates individual sets for each RP
- Uses meaningful naming: RP-Z_z2_y1_z5
...
def create_endpoint_rps():
    """
    Create RPs at endpoints of BarZ instances only.
    """
    
    print("=== CREATING BarZ ENDPOINT REFERENCE POINTS ===")
    print("Creating RPs at BarZ endpoints only with coordinate precision")
    print("")
    ...
    # Process BarZ instances only
    print("Processing BarZ instances...")
    barz_rps = process_bar_instances(assembly, 'BarZ', 'Z')
    
    # Summary
    print("")
    print("=== SUMMARY ===")
    print("BarZ endpoint RPs created: {}".format(barz_rps))
    
    if barz_rps > 0:
        print("SUCCESS: BarZ endpoint RPs created with perfect coordinate precision!")
        return True
    else:
...
def verify_endpoint_rps():
    """
    Verify that BarZ endpoint RPs were created correctly.
    """
    ...
    try:
        ...
        print("  RP-Z sets (BarZ endpoints): {}".format(len(rp_z_sets)))
        print("  Other RP sets: {}".format(len(other_rp_sets)))
        print("  Total BarZ endpoint RP sets: {}".format(len(rp_z_sets)))
        
        # Show examples
        if len(rp_z_sets) > 0:
            print("\nExample RP-Z sets:")
            for set_name in sorted(rp_z_sets)[:10]:
                print("  - {}".format(set_name))
            if len(rp_z_sets) > 10:
                print("    ... and {} more".format(len(rp_z_sets) - 10))
        
        # Verify a few sets contain RPs
        test_sets = rp_z_sets[:5]
...
def clean_existing_endpoint_rps():
    """
    Clean up existing BarZ endpoint RPs before creating new ones.
    """
    
    print("Cleaning existing BarZ endpoint RPs...")
    
    try:
        model = mdb.models['Model-1']
        assembly = model.rootAssembly
        
        # Find existing BarZ endpoint RP sets only
        endpoint_sets = []
        for set_name in assembly.sets.keys():
            if set_name.startswith('RP-Z_'):
        ...
        print("  Removed {} RP-Z sets and {} RPs".format(sets_removed, rps_removed))
        return True
...
def main():
    """Main function to create BarZ endpoint RPs."""
    
    print("BarZ ENDPOINT REFERENCE POINTS CREATION SCRIPT")
    print("=" * 50)
    print("Creates RPs at BarZ endpoints only with individual sets")
    print("")
    ...
    try:
        ...
        if success and total_rps > 0:
            print("\n" + "=" * 50)
            print("SUCCESS: BarZ endpoint RPs created successfully!")
            print("- {} RPs created at BarZ endpoints only".format(total_rps))
            ...
            print("\n" + "=" * 50)
            print("WARNING: BarZ endpoint RP creation issues")
