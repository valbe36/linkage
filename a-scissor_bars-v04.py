...
print("Grandstand: {} BarX rows x {} BarZ rows (tapering)".format(n_x+1, n_x))
...
try:
    ...
    print("FATAL ERROR: Model '{}' not found. Create it first or check name.".format(model_name))
    raise
...
def create_nonlinear_steel(model, material_name):
    if material_name not in model.materials:
        print("  Creating material: {}".format(material_name))
        try:
            ...
            print("  Material '{}' created.".format(material_name))
        except Exception as e:
            warnings.warn("Failed to create material '{}': {}".format(material_name, e))
            print(traceback.format_exc())
            raise
    else:
        print("  Material '{}' already exists.".format(material_name))
    ...
    for part_name in part_names:
        if part_name in model.parts:
            try:
                ...
                if not all_edges:
                    warnings.warn("No edges found for part '{}' during section assignment.".format(part_name))
                    success = False
                    continue
                ...
                print("  Assigned section '{}' to part '{}'.".format(section_name, part_name))
            except Exception as e:
                warnings.warn("Failed to assign section to part '{}': {}".format(part_name, e))
                print(traceback.format_exc())
                success = False
        else:
